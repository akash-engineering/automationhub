...
- **`Action`** — `workflowId : UUID`, `type : ActionType` (`SLACK | EMAIL | HTTP | DOCUMENT`, `@Enumerated(STRING)`, column `action_type`), `config : String` (JSON blob, `TEXT` column), `order : int` (column `execution_order` — `order` is a SQL reserved word).
...
- **Concrete executors** (`@Component` each):
  - `SlackActionExecutor`, `EmailActionExecutor` (in `workflow/`) — log the invocation and return success. User-facing notifications about workflow *outcomes* are the `notification` module's job, not these.
  - `HttpActionExecutor` (in `workflow/`) — uses Spring `RestClient`. Config is JSON: `{"url": "...", "method": "GET|POST|...", "body": <any json>}`. Non-2xx response or `RestClientException` → `ActionResult.failed`.
  - `DocumentActionExecutor` (in `document/service/action/`) — generates a PDF and persists a `Document` row. See `.claude/modules/document.md`. Proves the strategy pattern: a different module can plug in a new action type with zero edits to this registry or to existing executors.

**Adding a new action type** = (1) add an enum constant to `ActionType` (the only existing file you touch), (2) add a new `@Component implements ActionExecutor` *anywhere* in the codebase. The registry picks it up — do not edit it.

**Operational gotcha after adding an enum value:** the original DDL generated by Hibernate's `ddl-auto: update` includes a CHECK constraint listing the original enum values (`actions_action_type_check`). `update` mode adds new columns and tables but **does not update existing CHECK constraints**, so the first INSERT with the new value fails with `constraint [actions_action_type_check]`. Fix on an already-running DB: `ALTER TABLE actions DROP CONSTRAINT IF EXISTS actions_action_type_check;` — the JPA layer still enforces the enum via `@Enumerated(STRING)`. Long-term answer is Flyway/Liquibase (Increment 4).
