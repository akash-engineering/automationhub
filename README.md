...
- **payment** — Stripe-based subscription billing. `POST /checkout-sessions` creates a Stripe Checkout session; `POST /webhooks/stripe` verifies signatures via the Stripe SDK and applies events idempotently (unique constraint on `stripe_event_id`). Handles `checkout.session.completed`, `invoice.payment_succeeded`, `invoice.payment_failed`, `customer.subscription.deleted`. On a successful payment publishes `PaymentSucceededEvent`, which the notification module consumes (AFTER_COMMIT + @Async). Live Stripe calls are guarded by `STRIPE_API_KEY` — the app boots and tests pass with it unset.
